...
### [Core] 新增部分功能 <small>by @wibus-wee</small>

- 自定义发布活动与返回接口，你将可以通过定义发布活动来调度你自己的服务
- 自定义数据模型，你将可以通过定义数据模型来定义你自己的数据结构，但是此项功能仍处于测试阶段
- 自定义定时任务支持
- 静态资源管理服务支持
- 数据备份恢复支持，现在 Mog 已支持导入外部数据
- 友链服务支持获取全部状态的友链数据
